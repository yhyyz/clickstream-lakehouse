...
Deploy ECS service with ALB+Nginx+Vector clickstream processing containers.
...
OPTIONS:
    ...
    --msk-topic TOPIC                MSK topic name (default: $MSK_TOPIC)
    -h, --help                       Show this help message

EXAMPLES:
    # Basic deployment
    $0 --vpc vpc-12345678 --kafka-broker-host my-kafka-host.amazonaws.com

    # Advanced deployment with custom settings
    $0 --region us-west-2 --cluster my-cluster --vpc vpc-12345678 \\
...
NOTES:
    ...
    - Vector will use EBS volume mounted at /var/lib/vector for data caching
...
create_default_security_group() {
    local vpc_id="$1"
    local sg_name="clickstream-alb-ecs-sg"
...
EOF
    ...
    # Create ECS Task Role (with CloudWatch permissions)
...
EOF
        ...
        aws iam put-role-policy \
            --role-name "$task_role_name" \
            --policy-name "ClickstreamALBTaskPolicy" \
...
create_ecs_cluster() {
    ...
    # Check if cluster exists by looking for it in the response
  ...
  "containerDefinitions": [
    {
      ...
      "healthCheck": {
        "command": [
          "CMD-SHELL",
          "curl -f http://localhost:8802/health || exit 1"
        ],
        "interval": 30,
        "timeout": 5,
        "retries": 3,
    ...
    },
    {
      ...
      "environment": [
        ...
        },
        {
          ...
          "value": "$KAFKA_BROKER_HOST:$KAFKA_BROKER_PORT"
        },
        {
          "name": "AWS_MSK_TOPIC",
          "value": "$MSK_TOPIC"
      ...
      "mountPoints": [
        {
          "sourceVolume": "vector-ebs-volume",
...
main() {
    echo "=== ECS ALB+Nginx+Vector Clickstream Deployment Script ==="
    echo ""
